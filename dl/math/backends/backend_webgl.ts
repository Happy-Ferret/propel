...
export class MathBackendWebGL implements MathBackend {
  private texMap: WeakMap<DataId, TextureData> = new WeakMap();
  private canvas: HTMLCanvasElement;

  register(dataId: DataId, shape: number[], dtype: DataType): void {
    if (this.texMap.has(dataId)) {
      throw new Error(`data id ${dataId} already registered`);
    }
    this.texMap.set(dataId, {
      shape,
      dtype,
      values: null,
      texture: null,
  ...
  writePixels(
    ...
    const texture = this.texMap.get(dataId).texture ||
        this.textureManager.acquireTexture(texShape);
    const {shape} = this.texMap.get(dataId);
  ...
  write<D extends DataType>(dataId: DataId, values: DataTypeMap[D]): void {
    ...
    const {texture, texShape} = this.texMap.get(dataId);
    if (texture != null) {
      ...
      this.texMap.get(dataId).texture = null;
      this.texMap.get(dataId).texShape = null;
      this.texMap.get(dataId).textureType = null;
    }
    this.texMap.get(dataId).values = values;
  }

  readSync<D extends DataType>(dataId: DataId): DataTypeMap[D] {
    this.throwIfNoData(dataId);
    const {texture, values, textureType, texShape, numChannels} =
        this.texMap.get(dataId);
    if (values != null) {
    ...
    return this.texMap.get(dataId).values;
  }
  async read<D extends DataType>(dataId: DataId): Promise<DataTypeMap[D]> {
    this.throwIfNoData(dataId);
    const {texture, values, textureType, texShape} = this.texMap.get(dataId);
    if (values != null) {
      this.cacheOnCPU(dataId);
      return values;
    }
  ...
  disposeData(dataId: DataId): void {
    if (this.texMap.has(dataId)) {
      ...
      this.texMap.delete(dataId);
    }
  }

  getTexture(dataId: DataId): WebGLTexture {
    this.uploadToGPU(dataId);
    return this.texMap.get(dataId).texture;
  }

  getTextureData(dataId: DataId): TextureData {
    this.uploadToGPU(dataId);
    return this.texMap.get(dataId);
  }
  ...
  clone<D extends DataType, T extends NDArray<D>>(x: T): T {
    ...
    const {texShape} = this.texMap.get(x.dataId);
  ...
  private compileAndRun<T extends NDArray, K extends NDArray>(
    ...
    const inputsData: Array<ArrayData<T>> = inputs.map(input => {
      ...
      return {array: input, texData: this.texMap.get(input.dataId)};
    });
    this.uploadToGPU(output.dataId);
    const outputData = {array: output, texData: this.texMap.get(output.dataId)};
  ...
  private uploadToGPU(dataId: DataId): void {
    this.throwIfNoData(dataId);
    const {shape, values, texture, dtype} = this.texMap.get(dataId);
    if (texture != null) {
    ...
    this.texMap.get(dataId).textureType = TextureType.DEFAULT;
    this.texMap.get(dataId).texShape = texShape;
    const newTexture = this.textureManager.acquireTexture(texShape);
    this.texMap.get(dataId).texture = newTexture;
    if (values != null) {
  ...
  private cacheOnCPU(dataId: DataId, float32Values?: Float32Array) {
    ...
    const {texture, texShape, dtype} = this.texMap.get(dataId);
    if (texture != null) {
      this.textureManager.releaseTexture(texture, texShape);
      Object.assign(this.texMap.get(dataId), {
        texture: null,
        texShape: null,
        textureType: null,
      });
    ...
    if (float32Values != null) {
      this.texMap.get(dataId).values = float32ToTypedArray(float32Values, dtype);
    }
  }
}
